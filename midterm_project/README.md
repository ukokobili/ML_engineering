...
If you're interested in learning more about the project and the rearch, I wrote a short blog-post explaining more about it as well as my methods for this project.

Dataset Information
The data I use throughout this project was sourced from Google's Dataset search. It is available for download from several websites. The version I used is from Zenodo. There may be minor differences in data from other websites.

The research that uses and explains this data was published in The American Naturalist Volume 179, in January of 2012.

There is no need to download the data to run this repository as the data files have been supplied in the data folder.
...
Python Installation
Docker Installation
Setting up Docker and Windows Linux Subsystem - For Windows Users
Setting up local environment
Any code instructions listed throughout this guide are for a Windows system. Where possible, links to tutorials detailing the same or similar steps in other OS have been provided.

To run this repository on your local system, you will need to setup a Python virtual environment. I use the built-in Python venv for this. Python documentation has an extensive guide on creating these environments.
...
NOTE: Due to the specific nature of the data used in training this model, it performs accurately only if the data is within specific bounds. Below is the acceptable range of values.
...
This will also require an active installation of Gcloud CLI. A detailed guide on installing and running this can be found on Google Documentation.

None of the steps followed here will incur a charge on their own, and billing options merely require activation for verification purposes. Ensure you deactivate unnecessary processes to prevent accidental billing.
...
Start by navigating to the Google Cloud console and creating a new project. Make sure billing options are activated for this project.

Enable Artifact Registry.

We need the artifact registry to get our Docker container over to Google Cloud Run. Enable the API by following this link and completing the steps.

The user may require special permissions to run Docker. If at any point you run into permission issues, try again from an administrative user.
...
If you would like a more in-depth reading about the above, or need to find help for a different OS, consult these docs.
...
Navigate to the Google Cloud Run dashboard. Click on Create Service.
...
Make sure you pick the authentication option, Allow unauthenticated invocations. This will allow us to query it with the Streamlit app and the request.py script.
...
Configure Streamlit app and request.py.
